...
- the **jobs it covers** — usually one, but an interview that discusses several openings at once (two roles on the same team, say) can be linked to each of them: it then appears on every linked job's Interviews page with the same notes and Q&A, question generation considers all of them, and deleting a job only unlinks it (an interview with no job left goes with it);
