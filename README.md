...
| `research_company` | Have the server research a company on the web (Anthropic API) and propose its profile; `apply: true` saves it |
...
Each company has a page (click its name on a job) with its **website**, **company type**, **employee count**, notes, and referrals. **✨ Research with Claude** on that page has Claude search the web for the company (it needs the same `ANTHROPIC_API_KEY` as document generation, see below) and come back with a proposal: values for those three fields, shown next to what is saved now, plus a short briefing — what the company does, size and ownership, engineering signals, employer reputation, recent news — with its sources. Nothing changes until you click **Apply**, which fills the fields and appends the dated briefing to the notes; **Discard** throws it away. A confidence badge appears when Claude wasn't sure it found the right company (the tracked job postings' URLs are given to it to disambiguate). The same research is available to Claude through the `research_company` MCP tool and to scripts via `POST /api/company/research`.
...
- `POST /api/company/research?name=<name>` — Claude researches the company on the web (slow; needs the Anthropic key) and returns proposed `website`, `company_type`, `employee_count`, `headquarters`, `founded`, `confidence`, `summary`, `sources`, the `note_section` Apply would append, and the `current` field values; body `{ "apply": true }` saves the proposal and adds the updated `company`
