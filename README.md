...
| `add_interview` | Record an interview for a job (type, time, attendees by name — contacts are created as needed — and questions to ask) |
| `update_interview` | Change type/time/notes (replace or append), add or remove attendees, add questions, record answers, delete questions |
...
Once a job is `Interviewing` (or has interviews recorded), a **🎤 Interviews** link appears under its status. It opens the **Interviews page** for that job: a review of the job (posting link, level, salary, why it fits, notes, what you said in the application, who referred you) and of the company (profile, notes, and its standing interview-questions list) at the top, then the job's **interviews** — as many as the process has: recruiter screen, hiring manager, technical, system design, behavioral, panel, executive, team fit, final, or any custom type — as tabs. Each interview has:

- a **type** and **when** it takes place;
- **attendees** — picked from your contacts or created on the spot (a new attendee defaults to the job's company);
...
- `GET|PATCH|DELETE /api/interviews/:id` — one interview (PATCH: `type`, `scheduled_at`, `notes`)
