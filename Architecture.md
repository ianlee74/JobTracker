...
| [server/research.js](server/research.js) | Company research via the Anthropic API's web search, driven by the `research-company` skill — proposes profile fields, interview questions + a briefing, applies on request; job-posting parsing (`research-job` skill) that proposes the add-job form's fields and adds + researches a company the posting names that isn't tracked yet | db.js, generate.js |
...
| [skills/](skills/tailored-resume/SKILL.md) | Prompt instructions for document generation, company research, and job-posting parsing, as editable Markdown | — |
