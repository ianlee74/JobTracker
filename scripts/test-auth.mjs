...
// --- interviews & contacts (a user runs their own interviews; contacts are shared) ---
const iv = await jsonBody(`/api/jobs/${aliceJob.id}/interviews`, { method: 'POST', body: JSON.stringify({ type: 'recruiter', scheduled_at: '2026-09-22T14:00' }), ...H(aliceCookie) });
await check('user can add an interview to own job (type normalized)', { type: 'Recruiter', scheduled_at: '2026-09-22T14:00', attendees: [], questions: [] }, Promise.resolve({ type: iv.type, scheduled_at: iv.scheduled_at, attendees: iv.attendees, questions: iv.questions }));
await check('user cannot add an interview to another job -> 404', 404, status(`/api/jobs/${defaultJob.id}/interviews`, { method: 'POST', body: '{}', ...H(aliceCookie) }));
await check('interviews page data', { interviews: 1, company: 'Globex', hasTypes: true }, jsonBody(`/api/jobs/${aliceJob.id}/interviews`, H(aliceCookie)).then(d => ({ interviews: d.interviews.length, company: d.company.name, hasTypes: d.types.includes('Technical') })));
await check('job list carries interview_count', 1, jsonBody('/api/jobs', H(aliceCookie)).then(j => j.find(x => x.id === aliceJob.id).interview_count));
await check('user can create-and-attach an attendee (company defaults to the job\'s)', ['Jane Recruiter@Globex'], jsonBody(`/api/interviews/${iv.id}/attendees`, { method: 'POST', body: JSON.stringify({ contact: { name: ' Jane Recruiter ', title: 'Recruiter' } }), ...H(aliceCookie) }).then(i => i.attendees.map(c => `${c.name}@${c.company}`)));
const jane = (await jsonBody('/api/contacts?company=globex', H(aliceCookie)))[0];
await check('contacts are listed with interview counts', { name: 'Jane Recruiter', interview_count: 1 }, Promise.resolve({ name: jane.name, interview_count: jane.interview_count }));
await check('duplicate contact at the same company -> 409', 409, status('/api/contacts', { method: 'POST', body: JSON.stringify({ name: 'jane recruiter', company: 'GLOBEX' }), ...H(adminCookie) }));
await check('same name at another company is fine -> 201', 201, status('/api/contacts', { method: 'POST', body: JSON.stringify({ name: 'Jane Recruiter', company: 'Hooli' }), ...H(aliceCookie) }));
await check('user can edit a contact', 'Lead Recruiter', jsonBody(`/api/contacts/${jane.id}`, { method: 'PATCH', body: JSON.stringify({ title: 'Lead Recruiter' }), ...H(aliceCookie) }).then(c => c.title));
await check('user cannot delete a contact -> 403', 403, status(`/api/contacts/${jane.id}`, { method: 'DELETE', ...H(aliceCookie) }));
await check('attaching an existing contact by id', 2, jsonBody(`/api/interviews/${iv.id}/attendees`, { method: 'POST', body: JSON.stringify({ contact_id: jane.id }), ...H(aliceCookie) }).then(i => i.attendees.length).then(async n => n + (await jsonBody(`/api/interviews/${iv.id}/attendees`, { method: 'POST', body: JSON.stringify({ contact: { name: 'Bob Manager' } }), ...H(aliceCookie) })).attendees.length - 1));
await check('detaching an attendee keeps the contact', { attendees: ['Jane Recruiter'], contactExists: true }, jsonBody(`/api/interviews/${iv.id}/attendees/${(await jsonBody('/api/contacts?q=bob', H(aliceCookie)))[0].id}`, { method: 'DELETE', ...H(aliceCookie) }).then(async i => ({ attendees: i.attendees.map(c => c.name), contactExists: (await jsonBody('/api/contacts?q=bob', H(aliceCookie))).length === 1 })));
await check('questions append, tidy and dedupe', ['Why is the role open?', 'what does success look like?'], jsonBody(`/api/interviews/${iv.id}/questions`, { method: 'POST', body: JSON.stringify({ questions: ['- Why is the role open?', 'what does success look like?', 'why is the role open?', ''] }), ...H(aliceCookie) }).then(r => r.questions.map(q => q.question)));
const qs = (await jsonBody(`/api/interviews/${iv.id}`, H(aliceCookie))).questions;
await check('answers are recorded', 'Backfill.', jsonBody(`/api/interviews/${iv.id}/questions/${qs[0].id}`, { method: 'PATCH', body: JSON.stringify({ answer: 'Backfill.' }), ...H(aliceCookie) }).then(q => q.answer));
await check('emptying a question -> 400', 400, status(`/api/interviews/${iv.id}/questions/${qs[0].id}`, { method: 'PATCH', body: JSON.stringify({ question: '  ' }), ...H(aliceCookie) }));
await check('questions reorder', [qs[1].id, qs[0].id], jsonBody(`/api/interviews/${iv.id}/questions`, { method: 'PATCH', body: JSON.stringify({ order: [qs[1].id] }), ...H(aliceCookie) }).then(r => r.questions.map(q => q.id)));
await check('notes are saved (Markdown kept verbatim)', '## Prep\n- **bold**', jsonBody(`/api/interviews/${iv.id}`, { method: 'PATCH', body: JSON.stringify({ notes: '## Prep\n- **bold**' }), ...H(aliceCookie) }).then(i => i.notes));
await check('admin can read any interview', 200, status(`/api/interviews/${iv.id}`, H(adminCookie)));
// --- one interview covering several jobs ---
const aliceJob2 = (await jsonBody('/api/jobs', H(aliceCookie))).find(j => j.url === 'https://globex.example/3');
await check('interview starts with its one job', [aliceJob.id], Promise.resolve(iv.jobs.map(j => j.id)));
await check('user can link another own job', [aliceJob.id, aliceJob2.id].sort(), jsonBody(`/api/interviews/${iv.id}/jobs`, { method: 'POST', body: JSON.stringify({ job_id: aliceJob2.id }), ...H(aliceCookie) }).then(i => i.jobs.map(j => j.id).sort()));
await check('linking another person\'s job -> 404', 404, status(`/api/interviews/${iv.id}/jobs`, { method: 'POST', body: JSON.stringify({ job_id: defaultJob.id }), ...H(aliceCookie) }));
await check('admin linking a job of another person -> 400', 400, status(`/api/interviews/${iv.id}/jobs`, { method: 'POST', body: JSON.stringify({ job_id: defaultJob.id }), ...H(adminCookie) }));
await check('the interview shows on both jobs', [1, 1], Promise.all([jsonBody(`/api/jobs/${aliceJob.id}/interviews`, H(aliceCookie)), jsonBody(`/api/jobs/${aliceJob2.id}/interviews`, H(aliceCookie))]).then(ds => ds.map(d => d.interviews.filter(i => i.id === iv.id).length)));
await check('both jobs count it', [1, 1], jsonBody('/api/jobs', H(aliceCookie)).then(js => [aliceJob.id, aliceJob2.id].map(id => js.find(j => j.id === id).interview_count)));
await check('unlinking the primary job promotes the other', { jobs: [aliceJob2.id], job_id: aliceJob2.id }, jsonBody(`/api/interviews/${iv.id}/jobs/${aliceJob.id}`, { method: 'DELETE', ...H(aliceCookie) }).then(i => ({ jobs: i.jobs.map(j => j.id), job_id: i.job_id })));
await check('the last job cannot be unlinked -> 400', 400, status(`/api/interviews/${iv.id}/jobs/${aliceJob2.id}`, { method: 'DELETE', ...H(aliceCookie) }));
await check('relinking is idempotent', 2, jsonBody(`/api/interviews/${iv.id}/jobs`, { method: 'POST', body: JSON.stringify({ job_id: aliceJob.id }), ...H(aliceCookie) }).then(() => jsonBody(`/api/interviews/${iv.id}/jobs`, { method: 'POST', body: JSON.stringify({ job_id: aliceJob.id }), ...H(aliceCookie) })).then(i => i.jobs.length));
await check('deleting a linked job keeps the interview for the other', { exists: 200, jobs: [aliceJob.id] }, status(`/api/jobs/${aliceJob2.id}`, { method: 'DELETE', ...H(adminCookie) }).then(async () => ({ exists: await status(`/api/interviews/${iv.id}`, H(aliceCookie)), jobs: (await jsonBody(`/api/interviews/${iv.id}`, H(aliceCookie))).jobs.map(j => j.id) })));
await check('unknown interview -> 404', 404, status('/api/interviews/999999', H(adminCookie)));
await check('deleting a question', 1, jsonBody(`/api/interviews/${iv.id}/questions/${qs[1].id}`, { method: 'DELETE', ...H(aliceCookie) }).then(() => jsonBody(`/api/interviews/${iv.id}`, H(aliceCookie))).then(i => i.questions.length));
await check('deleting the interview clears its Q&A and count', { interviews: 0, count: 0 }, jsonBody(`/api/interviews/${iv.id}`, { method: 'DELETE', ...H(aliceCookie) }).then(async () => ({ interviews: (await jsonBody(`/api/jobs/${aliceJob.id}/interviews`, H(aliceCookie))).interviews.length, count: (await jsonBody('/api/jobs', H(aliceCookie))).find(x => x.id === aliceJob.id).interview_count })));
await check('admin can delete a contact', 200, status(`/api/contacts/${jane.id}`, { method: 'DELETE', ...H(adminCookie) }));
